...
addpath('C:\Users\mpiccardo\Dropbox (Harvard University)\IIT\Matlab\scripts\WaveFrontMatching_Code\')
...
% SLM specifications:
    ...
    PhaseScreenNum = 6; % Number of Phase screens used in optimization
...
% Setup parameters:
    ...
    w_in = 0.65e-3; % Input beam radius (m)
    w_out = 0.65e-3; % Output beam radius (m)
...
for jmodes = 1:Nmodes
    ...
    if SpotBasis
        ...
        ShiftRad = sqrt((X-SpotOutX0(jmodes)).^2+(Y-SpotOutY0(jmodes)).^2);
        ShiftAngle = angle((X-SpotOutX0(jmodes))+1i.*(Y-SpotOutY0(jmodes)))+pi; % Matrix with all angles starting left-center
        ModesOut(:,:,jmodes) = GenModesLG([0 0], w_out, ShiftRad, ShiftAngle);
    else
